...
contract Collection is Ownable, Pausable, ERC1155Holder {
  ...
  // Address of 's ERC1155 contract
  ERC1155 public ERC1155;

  constructor(address _ERC1155) public {
  ...
  function buyCollectable(uint256 _collectableIndex, uint256 _amount) payable external whenNotPaused {
    ...
    require(collectable.availableTokenAmount >= _amount, 'Collection: Not enough tokens available');
    require(_amount > 0, 'Collection: Purchase amount must be greater than 0');
    require(block.timestamp < roundEndTimestamp[roundId], 'Collection: Cannot buy after roundEndTimestamp');
    // Empty address indicates the collectable accepts ETH as payment
    if (collectable.ERC20Token == address(0)) {
      require(msg.value >= collectable.price, 'Collection: Insufficient fund to buy collectable');
    } else {
    ...
    ERC1155.safeTransferFrom(address(this), msg.sender, collectable.tokenId, _amount, "");
  }
  ...
  function addToCollection(
    ...
    uint256 _roundEndTimestamp) external onlyOwner {
      require(_amounts.length == _ERC20Tokens.length && _amounts.length == _prices.length, "Collection: Incorrect parameter length");
  ...
  function withdrawERC1155(uint256 _roundId, uint256 _collectableIndex, uint256 _amount) public onlyOwner {
    ...
    require(collectable.availableTokenAmount >= _amount, 'Collection: Not enough tokens available');
    require(block.timestamp >= roundEndTimestamp[_roundId], 'Collection: Can only withdraw after roundEndTimestamp');
    collectable.availableTokenAmount = collectable.availableTokenAmount.sub(_amount);
    ERC1155.safeTransferFrom(address(this), msg.sender, collectable.tokenId, _amount, "");
  }
  ...
  function withdrawAllERC1155(uint256 _roundId) external onlyOwner {
    for (uint256 i = 0; i < roundCollection[_roundId].length; i++) {
      ...
      uint256 ERC1155Balance = ERC1155.balanceOf(address(this), collectable.tokenId);
      if (ERC1155Balance > 0) {
