...
contract ERC1155 is AccessControl, ERC1155Pausable {
    ...
    function mintBatch(
        ...
        require(amounts.length == names.length && amounts.length == creatorNames.length && amounts.length == descriptions.length && amounts.length == mediaUris.length, "ERC1155: Incorrect parameter length");
